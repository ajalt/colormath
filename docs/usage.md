...
You can also convert to a specific color space with [convertTo][convertTo]:
...
[RGBToRGBConverter][RGBToRGBConverter], which is more efficient than using `convertTo` multiple times:
...
[multiplyAlpha][multiplyAlpha], and revert the operation with [divideAlpha][divideAlpha].
...
You can check if a color is within the sRGB gamut with [isInSRGBGamut][isInSRGBGamut].
...
In addition to [mostContrasting][mostContrasting], you can use
[firstWithContrast][firstWithContrast] or [firstWithContrastOrNull][firstWithContrastOrNull],
...
- [differenceCIE2000][differenceCIE2000]
- [differenceCMC][differenceCMC]
- [differenceEz][differenceEz]

## Gradients and Interpolation

For gradients and advanced interpolation, you can use the [interpolator][interpolator] builder. 
...
[EasingFunctions][EasingFunctions] includes all the CSS easing functions, as well as an easing
...
[HueAdjustments][HueAdjustments] contains all the methods defined in the CSS standard. By default,
[HueAdjustments.shorter][shorter] is used.
...
[createChromaticAdapter][createChromaticAdapter].
